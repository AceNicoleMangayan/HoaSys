...
<form id="submit_admin" method="post">
    <div class="modal fade" id="admin_modal" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" style="max-width: 800px" role="document">
            <div class="modal-content">
                            ...
                            <div class="col-xl-6">
                                <div class="form-group">
                                    ...
                                    <select
                                        ...
                                        <option value="due">Due Manager</option>
                                        <option value="election">Election Committee</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-metal" data-dismiss="modal">Cancel</button>&nbsp; <button
...
<form id="submit_admin_ho" method="post">
    <div class="modal fade" id="admin_modal_ho" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" style="max-width: 800px" role="document">
            <div class="modal-content">
                <div class="modal-body">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    ...
                    <div style="margin: 0 20px">
                        <div class="form-group m-form__group row">
                            <div style="margin: 20px auto;">
                                <h2 style="text-align: center!important;">Homeowner as Admin</h2>
                                <h5 style="text-align: center!important;" class="text-muted" id="subheader_ho">Add
                                    homeowner as admin</h5>
                            </div>
                        </div>
                        <div class="row">
                            <hr>
                            <div class="row">
                                <div class="col-xl-12 mb-4">
                                    <select class="form-control m-select2 m-input m-input--square"
                                        name="homeowners_options" id="homeowners_options">
                                    </select>
                                </div>
                                <div class="col-xl-6">
                                    <div class="form-group">
                                        <label for="recipient-name" class="m--font-bolder">Username <span
                                                class="m--font-danger">*</span></label>
                                        <input type="text" class="form-control m-input m-input--solid" id="username_ho"
                                            autocomplete="off" required>
                                    </div>
                                </div>
                                <div class="col-xl-6">
                                    <div class="form-group">
                                        ...
                                        <input type="email" class="form-control m-input m-input--solid"
                                            id="email_admin_ho" autocomplete="off" name="email_admin_ho" required>
                                    </div>
                                </div>
                                <div class="col-xl-6">
                                    <div class="form-group">
                                        ...
                                        <select
                                            class="form-control m-bootstrap-select m-bootstrap-select--solid m_selectpicker"
                                            title="Role" tabindex="-98" id="role_ho" required>
                                            <option value="admin">Admin</option>
                                            <option value="officer">Officer</option>
                                            <option value="due">Due Manager</option>
                                            <option value="election">Election Committee</option>
                                ...
                                <div class="col-xl-6" style="">
                                    <label for="recipient-name" class="m--font-bolder">Password <span
                                            class="m--font-danger">*</span></label>
                                    <input type="text" class="form-control m-input m-input--solid" id="password_ho"
                                        autocomplete="off" required>
                                </div>
                            </div>
                            <hr>
...
<div class="modal fade" id="admin_option_modal" data-type="0" data-id="0" role="dialog"
        ...
        <div class="modal-content">
            <div class="modal-body">
                ...
                <div style="margin: 0 20px">
                    <div class="form-group m-form__group row">
                        <div style="margin: 20px auto;">
                            <h3 id="billing_title" style="text-align: center!important;">CHOOSE TYPE OF ADMIN</h3>
                            <h5 style="text-align: center!important;" class="text-muted" id=""></h5>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xl-6 col-lg-12 col-md-12 col-sm-12 text-center">
                            <button id="admin_new_btn" type="button" class="btn btn-primary"> <span><i
                                        class="fa fa-user" style="font-size:50px;"></i></span></button>
                            <br><span><strong>+ ADD</strong> New Admin</span>
                        </div>
                        <div class="col-xl-6 col-lg-12 col-md-12 col-sm-12 text-center">
                            <button id="admin_homeowner_btn" type="button" class="btn btn-info"> <span><i
                                        class="fa fa-user" style="font-size:50px;"></i></span></button>
                            <br><span><strong>+ ADD</strong> Homeowner as Admin</span>
                        </div>
    ...
    var offense = function(searchVal = "") {
        var options = {
            ...
            columns: [{
                ...
                textAlign: 'left',
                template: function(row, index, datatable) {
                    let content = row.role;
                    if (content == "due") {
                        content = "Due Manager";
                    } else if (content == "election") {
                        content = "Election Commitee";
                    }
                    let html = "<span class='text-capitalize'>" + content + "</span>";
                    return html;
                }
            }, {
                field: "status",
...
function open_admin_options() {
    $("#admin_option_modal").modal("show");
}
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data === 1) {
                ...
                swal("Error!", "The Username is already in use! Try another.", "error");
            } else if (data === 4) {
                swal("Error!", "The Email is already in use!Try another.", "error");
            } else {
                swal("Error!", "Contact Adminstrator!", "error");
            }
        }
    })
})
...
$("#admin_new_btn").on("click", function() {
    $('#change_password_input').show();
    $('#change_button').hide();
    $('#cancel_button').hide();
    $("#admin_option_modal").modal('hide');
    $('#admin_modal').modal('show');
    $('#subheader').text('Create New Admin');
    $('#action').val('create');
    $('#password').show();
...
$("#admin_homeowner_btn").on("click", function() {
    $("#admin_option_modal").modal('hide');
    $("#admin_modal_ho").modal('show');
    ho_toadd_options();
});

function ho_toadd_options() {
    $("#homeowners_options").select2({
        ajax: {
            url: "<?= base_url(); ?>admin/homeowners_to_admin_select",
            dataType: "json",
            type: "POST",
            delay: 250,
...
$('#submit_admin_ho').on('submit', (e) => {
    e.preventDefault()
    let ho_id = $('#homeowners_options').val();
    let username = $('#username_ho').val();
    let password = $('#password_ho').val();
    let role = $('#role_ho').val();
    let email = $("#email_admin_ho").val();
    $.ajax({
        type: "POST",
        url: "<?php echo base_url(); ?>admin/admin_ho_saving",
        dataType: "JSON",
        data: {
            ho_id,
            username,
        ...
        success: function(data) {
            if (data === 1) {
                swal("Success!", `Admin successfully added!`, "success");
                $('#adminData').mDatatable('reload');
                $('#admin_modal_ho').modal('hide');
            } else if (data === 2) {
                swal("Error!", "This Homeowner already added as admin!", "error");
            } else if (data === 3) {
                swal("Error!", "The Username is already in use! Try another.", "error");
            } else if (data === 4) {
                swal("Error!", "The Email is already in use!Try another.", "error");
